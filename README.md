...
* FOR LOCAL DEVELOPMENT AND TESTING: To run the gunicorn setup not through flask and nginx, you should first stop the flask_app `sudo systemctl stop flask_app` then go to `~/MedsynBackend/src/`, activate the medsyn-3-8 conda environment 'conda activate medsyn-3-8' if it's not activated then run this command `/home/exouser/miniconda3/envs/medsyn-3-8/bin/gunicorn --workers 1 --bind 0.0.0.0:5000 app:app`

## Finding where all the results are stored
* `ls /media/volume/gen-ai-volume//MedSyn/results/`