...
#             return out[0] + x, out[1]  # Add only the main tensor, pass attention map separately
#         return out + x
# ################################################
...
#         attn_logits = torch.einsum('b i d, b j d -> b i j', q * self.scale, k)
...
#         #######################################################
#         return out, attn_weights.squeeze(-1) #attn_weights  HERE
#     ###########################################################
...
#         super().__init__()
#         ####################################
#         self.cross_attention_maps = [] #here
#         self.khms_folder = khms_folder
...
#   ########################################################################      MINE
#         # Capture attention maps from multiple layers, append them to the list
#         x = self.mid_spatial_attn1(x)
#         x, attn_map1 = self.mid_cross_attn1(x, kv=cond)
#         self.cross_attention_maps.append(attn_map1)
#         print(f"cross_attention_maps length after mid_cross_attn1: {len(self.cross_attention_maps)}")
#         x = self.mid_temporal_attn1(x, t)

#         x = self.mid_spatial_attn2(x)
#         x, attn_map2 = self.mid_cross_attn2(x, kv=cond)
#         self.cross_attention_maps.append(attn_map2)
#         print(f"cross_attention_maps length after mid_cross_attn2: {len(self.cross_attention_maps)}")
#         x = self.mid_temporal_attn2(x, t)

#         x = self.mid_spatial_attn3(x)
#         x, attn_map3 = self.mid_cross_attn3(x, kv=cond)
#         self.cross_attention_maps.append(attn_map3)
#         print(f"cross_attention_maps length after mid_cross_attn3: {len(self.cross_attention_maps)}")
#         x = self.mid_temporal_attn3(x, t)

#         x = self.mid_spatial_attn4(x)
#         x, attn_map4 = self.mid_cross_attn4(x, kv=cond)
#         self.cross_attention_maps.append(attn_map4)
#         print(f"cross_attention_maps length after mid_cross_attn4: {len(self.cross_attention_maps)}")
...
#             # Collect every 4th map starting at i for each modality
#             maps_per_modality = self.cross_attention_maps[i::4]  
#             # Average across layers for the given modality
#             modality_avg = torch.mean(torch.stack(maps_per_modality), dim=0)  
#             reshaped_maps.append(modality_avg)

#         # Stack the averaged modality maps to create the final shape
#         final_attention_map = torch.stack(reshaped_maps).unsqueeze(0).cpu().numpy()  # Shape: (1, 4, 64, 64, 64)
#         heatmap_save_path = os.path.join(self.khms_folder, f"{sample_id}{self.sample_counter}.npy")
#         np.save(heatmap_save_path, final_attention_map)
#         self.sample_counter += 1
        
        
#         ###########################################################################################
#         # if self.cross_attention_maps:
#         #     print(f"Saving heatmaps to folder: {self.khms_folder}")
#         #     avg_attention_map = torch.mean(torch.stack(self.cross_attention_maps), dim=0).cpu().numpy()
#         #     heatmap_save_path = os.path.join(self.khms_folder, f"{sample_id}{self.sample_counter}.npy")
#         #     np.save(heatmap_save_path, avg_attention_map)
#         #     self.sample_counter += 1
#  ###################################################################################################          
...
#         return self.final_conv(x) #self.avg_cross_attention_map #addaed attetion maps 
...
#         x_recon = self.denoise_fn.forward_with_cond_scale(x, t, indexes=indexes, cond=cond, cond_scale=cond_scale) #cchaneg ADDED _
...
#         #avg_attention_map = self.denoise_fn.avg_cross_attention_map #added
...
# ######### Setup paths for KHM and Text Prompts folders####################
#         self.khms_folder = os.path.join(self.save_folder, "KHM_Results")
#         self.prompts_folder = os.path.join(self.save_folder, "Text_Prompts")
# ############################################################################
# ########## Ensure the directories are created#############
#         os.makedirs(self.khms_folder, exist_ok=True)
...
#                     ########### Additional paths for saving heatmaps and text prompts ###########
#                     heatmap_save_path = os.path.join(self.khms_folder, f"{file_name}_KHM.npy")  # Heatmap save path
#                     prompt_save_path = os.path.join(self.prompts_folder, f"{file_name}_prompt.txt")  # Text prompt save path
#                     ###########################################################################

#                     # all_heatmaps_list = [] #new
...
#                         #avg_heatmap = self.model.avg_cross_attention_map.cpu().numpy()
# #                         avg_heatmap = self.model.denoise_fn.avg_cross_attention_map
# #                         np.save(heatmap_save_path, avg_heatmap.cpu().numpy())
                        
#                         with open(prompt_save_path, 'w') as f:
#                             f.write(str(text.cpu().numpy()))
#                         ##############################################################
...
#         self.to_kv = nn.Linear(dim_con, hidden_dim*2, bias=False)
#         self.to_out = nn.Conv2d(hidden_dim, dim, 1)
#         # Initialize an empty list to hold attention maps
#         self.attention_maps = []
...
#         # Append the attention map for each slice to the list
#         self.attention_maps.append(hidden_states)
#         print(f"Debug: Heatmap appended. Current attention map length: {len(self.attention_maps)}")
...
#             mask = prob_mask_like((batch,), null_cond_prob, device=device)
#             null_cond_emb = torch.cat([self.null_cond_emb]*batch, dim=0)
#             #null_cond_emb_exp = self.null_cond_emb.expand(batch, *cond.shape[1:])
#             cond = torch.where(rearrange(mask, 'b -> b 1 1'), null_cond_emb, cond)
...
#         # Capture attention maps from the last cross-attention layer
#        # self.heatmaps = self.mid_cross_attn4.fn.fn.attention_maps
...
#         self.heatmaps = []  # Clear previous heatmaps
...
#                 # Store heatmaps generated in Unet3D for this timestep
#             if hasattr(self.denoise_fn, 'heatmaps') and isinstance(self.denoise_fn.heatmaps, list):
#                 print(f"Debug: Adding heatmaps from denoise_fn.heatmaps, length: {len(self.denoise_fn.heatmaps)}")
#                 self.heatmaps.append([h.clone().detach() if isinstance(h, torch.Tensor) else h for h in self.denoise_fn.heatmaps])
#                 print(f"Debug: Heatmap stack length after adding: {len(self.heatmaps)}")
#                 for idx, heatmap in enumerate(self.heatmaps[-1]):
#                     if isinstance(heatmap, torch.Tensor):
#                         print(f"Debug: Heatmap {idx} shape at timestep {t}: {heatmap.shape}")
...
#             for idx in range(self.num_sample):
#                 with torch.no_grad():
#                     self.ema_model.denoise_fn.heatmaps = []
...
#                         heatmaps = self.ema_model.denoise_fn.heatmaps
#                         if heatmaps:
#                             heatmap_output = torch.stack(heatmaps)
#                             heatmap_save_path = os.path.join(self.save_folder, f"{file_name}_heatmaps.npy")
#                             np.save(heatmap_save_path, heatmap_output.cpu().numpy())
...
# WORKING TOKEN WISE ATTENTION MAPS
import math
import copy
import torch
import numpy as np
...
class Unet3D(nn.Module):
    ...
    def forward(
       ...
       # self.heatmaps = self.mid_cross_attn4.fn.fn.attention_maps
        ...
        x = torch.cat((x, r), dim=1)
        print(f"Debug: Total attention maps collected in forward pass: {len(self.attention_maps)}")
...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample_loop(self, shape, cond=None, cond_scale=1., use_ddim=True):
        device = self.betas.device
        # Set a specific seed before sampling
        set_seed(100)  # Use the same fixed integer as above
        ...
        self.attention_maps = []  # Clear previous heatmaps
...
class Trainer(object):
    def __init__(
        ...
        super().__init__()
        self.model = diffusion_model
        self.tokenizer = tokenizer  # Assign the tokenizer
        map_location = 'cuda' if torch.cuda.is_available() else 'cpu'
        print(results_folder)
        model_path = os.path.join(results_folder,"1000_ckpt/pytorch_model.bin")
        self.model.load_state_dict(torch.load(model_path, map_location=map_location), strict=False)
...
#         self.ds = cache_transformed_text(train_files=train_files)
    ...
    def train(
        ...
        for i, data in enumerate(self.dl):
            ...
            filenames = data["filename_or_obj"]

            for idx in range(self.num_sample):
                with torch.no_grad():
                    self.ema_model.attention_maps = []

                    file_name = filenames[0].split('/')[-1].split('.')[0] + f"_sample_{idx}.npy"
                    ...
                    if not os.path.exists(save_path):
                        ...
                        all_videos_list = list(
                            map(lambda n: self.ema_model.sample(batch_size=n, cond=text), batches))
                        all_videos_list = torch.cat(all_videos_list, dim=0)
                        np.save(save_path, all_videos_list.cpu().numpy())
                        ...
                        if heatmaps:
                            ...
                            for token_idx in range(tokens.shape[1]):
                                ...
                                heatmap_save_path = os.path.join(self.save_folder, f"{file_name}_token_{token_idx}_{token_str}_heatmaps.npy")
                                np.save(heatmap_save_path, attention_maps_per_frame)
                                print(f"Saved heatmap for token '{token_str}' at: {heatmap_save_path}")
                        else:
                            print("No heatmaps to save.")
                    else:
...
#                         heatmaps = self.ema_model.attention_maps
#                         if heatmaps:
#                         # Stack attention maps over time steps and layers
#                             all_attention_maps = []
#                             for timestep_idx, timestep_maps in enumerate(heatmaps):
#                                 print(f"Debug: Timestep {timestep_idx}, Number of attention maps: {len(timestep_maps)}")
#                                 # Each timestep_maps is a list of attention maps from all cross-attention layers
#                                 # Each attention map is of shape [batch_size * frames, heads, query_len, key_len]
#                                 # We need to stack over layers
#                                 layer_maps = torch.stack(timestep_maps)  # Shape: [num_layers, batch_size * frames, heads, query_len, key_len]
#                                 # Average over layers at this time step
#                                 avg_layer_map = layer_maps.mean(dim=0)  # Shape: [batch_size * frames, heads, query_len, key_len]
#                                 all_attention_maps.append(avg_layer_map)
                        
#                             # Stack over time steps
#                             all_attention_maps = torch.stack(all_attention_maps)  # Shape: [num_timesteps, batch_size * frames, heads, query_len, key_len]

#                             # Average over time steps
#                             avg_attention_map = all_attention_maps.mean(dim=0)  # Shape: [batch_size * frames, heads, query_len, key_len]

#                             # Save the averaged attention map without reshaping or normalization
#                             heatmap_save_path = os.path.join(self.save_folder, f"{file_name}_heatmaps.npy")
#                             np.save(heatmap_save_path, avg_attention_map.cpu().numpy())
#                             print(f"Saved heatmap at: {heatmap_save_path}")
#                         else:
#                             print("No heatmaps to save.")
...
#         # Extract the CLS token attention. CLS is typically at index 0.
#         # This selects the attention weights for the CLS token across all heads and query positions.
#         attn_cls = attn_weights[..., 0]  # shape: [batch, heads, query_len]
        
#         # Average over heads to get a single attention map per query position
#         attn_cls_single = attn_cls.mean(dim=1)  # [batch, query_len]
        
#         # Store this CLS-based attention map
#         self.attention_maps.append(attn_cls_single.detach().cpu())
...
#                             # Originally, code tries to average over heads/tokens and reshape query_len, key_len.
#                             # Remove any loops or references to `token_idx`, `tokens`, or `key_len`.
#                             # We now have [num_layers, batch_size*frames, query_len] maps at each timestep.
                        
#                             avg_attention_maps_per_timestep = []
#                             for timestep_idx, timestep_maps in enumerate(heatmaps):
#                                 # timestep_maps: list of attention maps for each layer at this timestep
#                                 # Each element in timestep_maps is now of shape [batch_size*frames, query_len]
#                                 layer_maps = torch.stack(timestep_maps)  # [num_layers, batch_size*frames, query_len]
                        
#                                 # Average over layers
#                                 avg_layer_map = layer_maps.mean(dim=0)  # [batch_size*frames, query_len]
                        
#                                 # Reshape to [batch_size, frames, query_len]
#                                 avg_layer_map = avg_layer_map.view(batch_size, frames, avg_layer_map.shape[1])
#                                 avg_attention_maps_per_timestep.append(avg_layer_map)
                        
#                             # Stack over time steps: [num_timesteps, batch_size, frames, query_len]
#                             time_attention_maps = torch.stack(avg_attention_maps_per_timestep)
                        
#                             # Average over time steps: [batch_size, frames, query_len]
#                             avg_attention_map = time_attention_maps.mean(dim=0)
                        
#                             # Convert query_len to spatial dims h, w
#                             query_len = avg_attention_map.shape[2]
#                             h = w = int(math.sqrt(query_len))
#                             avg_attention_map_reshaped = avg_attention_map.view(batch_size, frames, h, w).cpu().numpy()
                        
#                             # Save the CLS-based attention heatmap
#                             heatmap_save_path = os.path.join(self.save_folder, f"{file_name}_cls_heatmap.npy")
#                             np.save(heatmap_save_path, avg_attention_map_reshaped)
#                             print(f"Saved CLS-based attention heatmap at: {heatmap_save_path}")
#                         else:
#                             print("No heatmaps to save.")
#                     else:
#                         print(f"File already exists: {save_path}")
