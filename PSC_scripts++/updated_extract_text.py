...
class TextExtractor:
    def __init__(self, 
        ...
        ckpt = torch.load(os.path.join(self.resume_model, "pytorch_model.bin"), map_location=self.device)
    ...
    def run(self, text, output_folder, file_name):
        ...
        if os.path.exists(file_path) and os.path.exists(tokens_path):
            print("Files already exist: " + file_path + " and " + tokens_path)
            return
